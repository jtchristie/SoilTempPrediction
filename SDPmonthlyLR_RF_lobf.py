...
season_mapping = {1: 'cold', 2: 'cold', 3: 'cold', 4: 'warm', 5: 'warm', 6: 'warm', 7: 'warm', 8: 'warm', 9: 'warm',
                  10: 'cold', 11: 'cold', 12: 'cold'}
...
# Line of best fit for Warm Season - Linear Regression (LR)
LR_coeffs_warm = np.polyfit(Y_test_warm, LRpredict_warm, 1)
LR_line_warm = np.polyval(LR_coeffs_warm, Y_test_warm)
axs[0].plot(Y_test_warm, LR_line_warm, color='blue')

# Line of best fit for Warm Season - Random Forest (RF)
RF_coeffs_warm = np.polyfit(Y_test_warm, rf_predict_warm, 1)
RF_line_warm = np.polyval(RF_coeffs_warm, Y_test_warm)
axs[0].plot(Y_test_warm, RF_line_warm, color='red')

# Line of best fit for Cold Season - Linear Regression (LR)
LR_coeffs_cold = np.polyfit(Y_test_cold, LRpredict_cold, 1)
LR_line_cold = np.polyval(LR_coeffs_cold, Y_test_cold)
axs[1].plot(Y_test_cold, LR_line_cold, color='blue')

# Line of best fit for Cold Season - Random Forest (RF)
RF_coeffs_cold = np.polyfit(Y_test_cold, rf_predict_cold, 1)
RF_line_cold = np.polyval(RF_coeffs_cold, Y_test_cold)
axs[1].plot(Y_test_cold, RF_line_cold, color='red')
